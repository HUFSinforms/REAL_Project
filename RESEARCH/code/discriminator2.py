...
class Discriminator(nn.Module):
    def __init__(self, in_dim=500,train=True, risk_mat=[], bw=[], alpha=[], nbsector=0 , nbmq=0,w_11=0.5,w_lambda=1.,c_lower=50.):
        ...
        self.card_upper = len(bw)*0.2
        self.card_lower = len(bw)*0.1
    ...
    def forward(self, x):
        ...
        # (5)
        tot_infs += F.relu(torch.abs(x-x_bw)@ Variable(torch.ones(self.in_dim))-0.3)
        ...
        tot_infs += F.relu(dQd - 0.1)*self.w_11
        tot_infs += F.relu(0.05 - dQd)*self.w_11
